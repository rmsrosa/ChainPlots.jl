...
properties:
    `:layer_number`: indicates to each layer it belongs (an `Int` value 
        with`0` indicating the input layer, `1, …, length(m)-1` 
        indicating the hidden layers, and with `length(m)` indicating
        the output layer)
    `:layer_type`: indicates the layer type it is part of in the Chain
    ...
    `:layer_center`: The vertical mid-point of the layer it belongs to.
"""
    ...
    for i in 1:length(node_to_neuron)
        set_props!(
            mg, i, Dict(
                ...
                :layer_type => first(node_to_neuron[i]) == 0 ? :input_layer : m[first(node_to_neuron[i])],
